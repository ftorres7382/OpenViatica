...
from OpenViatica._types import ov_ws_types, ov_ws_type_t, \
    meta_workspace_toml_type_value, templates_workspace_toml_type_value
...
class MetaWorkspace:
        ...
        def get_linked_workspace_object(
            self, identifier: str, identifier_type: t.Literal["id", "name"], expected_type_name: t.Literal["ov-meta"]
        ) -> "MetaWorkspace": ...

        # 2. Overload for "remote"
        @t.overload
        def get_linked_workspace_object(
            self, identifier: str, identifier_type: t.Literal["id", "name"], expected_type_name: t.Literal["ov-templates"]
        ) -> TemplatesWorkspace: ...

        # 3. Overload for None (returns the Base or a Union)
        @t.overload
        def get_linked_workspace_object(
            self, identifier: str, identifier_type: t.Literal["id", "name"], expected_type_name: None = None
        ) -> t.Union["MetaWorkspace", TemplatesWorkspace]: ...
        ...
        def get_linked_workspace_object(
            ...
            # Get the init args
            _workspace_toml_filename = os.path.basename(workspace_dict["workspace_tomlpath"])
            ...
            if expected_type_name is not None:
                if not isinstance(workspace_obj, WORKSPACE_CLASS_MAPPING_DICT[expected_type_name]):
                    raise TypeError(f"The type of the workspace '{identifier}' is not the same as the expected type: '{expected_type_name}'")
        ...
        def get_workspace_links_to_dict(
            ...
            # Try to filter
            filtered_values = [item for item in links_to_values if item[identifier_type] == identifier]

            # Check for any potential cases
            if len(filtered_values) == 0:
                raise ov_errors.LinkNotFoundError(f"Found no link with '{identifier_type}' equal to '{identifier}'")
            elif len(filtered_values) > 1:
                raise ov_errors.DuplicatedLinksFoundError(f"Found multiple links where the '{identifier_type}' equals '{identifier}'")
...
WORKSPACE_CLASS_MAPPING_DICT: t.Dict[
    ov_ws_type_t,
    t.Type[MetaWorkspace | TemplatesWorkspace]
] = {
