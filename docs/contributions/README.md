...
# Package Release
1. Remember to increase the version number as defined in the [project documentation](../project/README.md)
...
6. Type Check Standards: To maintain the project correctly, we will need to enforce certain type safety standards even in python.
    1. Project MUST pass ```mypy --strict``` check
    2. ALL external dynamic variables MUST use ```typeguard.check_type``` to validate their expected schema
...
- Bugfixes and documentation typo fixes are always a good icebreaker
- How to do contributions on new functionalities
    1. Decide on an issue for a module/function/section in the that you want to work on
    ...
    2. Fork this repo
    3. You can work on it as you see in your own fork, but I would recommend following the process that the main repo will do so that there is better parity
    4. When you are done with the changes, these are the steps to start incorporating the work into the main branch.
